...
def test_gate_passes(iris_model):
    ...
    ai_critic.gate(report, min_score=0.0)
...
def test_gate_blocks():
    ...
    report = ai_critic.audit(m, X, y)
    with pytest.raises(RuntimeError):
        ai_critic.gate(report, min_score=0.99)


def test_parallel_matches_sequential(iris_model):
    model, X, y = iris_model
    r_seq = ai_critic.audit(model, X, y, parallel=False)
    r_par = ai_critic.audit(model, X, y, parallel=True)
